...
export default function SkillsAdmin() {
  ...
  const onSave = async () => {
    try {
      ...
      setSaving(true);
      if (editing) {
        await updateSkill(editing._id, vals);
        toast.success("Skill updated!");
      } else {
        await createSkill(vals);
    ...
    <div>
      ...
      >
        <div>
          ...
          >
            {skills.length} skills across {Object.keys(grouped).length}{" "}
            categories
          </p>
        </div>
        <Button
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontFamily: "monospace",
                          fontSize: 11,
                          color: COLORS[sk.category] || "var(--accent)",
                        }}
                      >
                        {sk.level}%
                      </span>
                    ...
                    >
                      <div
                        style={{
                          height: "100%",
                          width: `${sk.level}%`,
                          background: COLORS[sk.category] || "var(--accent)",
                          borderRadius: 2,
                        }}
                      />
                    </div>
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item
            name="name"
            label={<span style={labelStyle}>Skill Name</span>}
            rules={[{ required: true }]}
          >
          ...
          <Form.Item
            name="level"
            label={<span style={labelStyle}>Proficiency Level</span>}
          >
            <Slider
              min={0}
              max={100}
          ...
          >
            <Form.Item
              name="color"
              label={<span style={labelStyle}>Color</span>}
            >
              <Input placeholder="#38bdf8" />
            </Form.Item>
            <Form.Item
              name="order"
              label={<span style={labelStyle}>Order</span>}
            >
